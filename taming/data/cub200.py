...
class CUBTrain(Base):
    def __init__(self, size, keys=None):
        super().__init__()
        root = '/opt/tiger/fnzhan/datasets/CUB-200'
        with open(root + "/lists/lists/train.txt", "r") as f:
            relpaths = f.read().splitlines()
        paths = [os.path.join(root + '/images/images', relpath) for relpath in relpaths]
        self.data = CUB200Base(paths=paths, size=size, random_crop=False)
        self.keys = keys

class CUBValidation(Base):
    def __init__(self, size, keys=None):
        super().__init__()
        root = '/opt/tiger/fnzhan/datasets/CUB-200'
        with open(root + "/lists/lists/test.txt", "r") as f:
            relpaths = f.read().splitlines()
        paths = [os.path.join(root + '/images/images', relpath) for relpath in relpaths]
        self.data = CUB200Base(paths=paths, size=size, random_crop=False)
        self.keys = keys

class CUB200Train(Dataset):
    def __init__(self, size, keys=None, crop_size=None):
        d1 = CUBTrain(size=size, keys=keys)
...
class CUB200Validation(Dataset):
    # CelebAHQ [0] + FFHQ [1]
    def __init__(self, size, keys=None, crop_size=None):
        d1 = CUBValidation(size=size, keys=keys)
